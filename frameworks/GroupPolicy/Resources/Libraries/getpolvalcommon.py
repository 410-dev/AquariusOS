...
def _get_locale(cur: sqlite3.Cursor, username: str) -> str:
    """
    로케일 캐스케이드 순서:
...
def init() -> tuple[int, sqlite3.Connection | str]:
...
def fetch_policy_data(username: str, policies_to_read: list[str]) -> tuple[int, list[dict] | str]:

    exit_code, conn_or_error = init()
    if exit_code != 0:
        return 1, conn_or_error  # DB 연결 실패 에러 메시지 반환
    ...
    with conn:
        ...
        locale = _get_locale(cur, username)
