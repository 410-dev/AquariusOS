...
def simple_reg_loader(key: str, rtype: str, default):
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = f.read()
            # Types: dword, qword, str, list, bool
            if rtype == "dword":
                return int(data)
            elif rtype == "qword":
                return int(data)
...
ProbingIntervalSeconds: int = simple_reg_loader("ProbingIntervalSeconds", "dword", 3)
GlobalApplicationProbeTargets: list[str] = simple_reg_loader("GlobalApplicationProbeTargets", "list", ["/applications", "/opt/applications"])
RegistryDir: str = simple_reg_loader("RegistryDir", "str", "/var/lib/apprun")
RegistryFile = simple_reg_loader("RegistryFile", "str", "desktop-links.json")
BackendOverride: str = simple_reg_loader("BackendOverride", "str", "polling")

# Updating config requires restart of the service
CONFIG = {
    "MakeDirectoryIfPossible": MakeDirectoryIfPossible,
    "BaseDirectory": BaseDirectory,
    "ApplicationsDirectory": ApplicationsDirectory,
    "ProbingIntervalSeconds": ProbingIntervalSeconds,
    "GlobalApplicationProbeTargets": GlobalApplicationProbeTargets,
...
def _registry_path() -> str:
    reg_dir = CONFIG.get("RegistryDir")
    reg_file = CONFIG.get("RegistryFile")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backend", choices=["polling", "inotify-experimental"],
                        default=BackendOverride, help="Choose the monitoring backend")
